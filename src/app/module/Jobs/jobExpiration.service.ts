 ...
 */
export const sendJobExpiringNotifications = async (
  ...
  for (const job of expiringJobs) {
    ...
    try {
      await notificationsService.createNotification(
        job.postedBy.id,
        "JOB_EXPIRING_SOON",
        `Job expiring soon — ${job.title}`,
        `Your job posting ${job.title} at ${job.company.name} is expiring on ${job.expiresAt?.toISOString()}`,
