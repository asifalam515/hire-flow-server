...
const createWorkExperience = asyncHandler(
...
const getMyWorkExperiences = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) throw new AppError("Authentication required", 401);
    ...
    const experiences =
      await workExperienceService.getWorkExperienceByProfileId(profile.id);

    res.status(200).json({ success: true, data: experiences });
  },
);

const updateWorkExperience = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    const { id } = req.params;
...
const deleteWorkExperience = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    const { id } = req.params;
