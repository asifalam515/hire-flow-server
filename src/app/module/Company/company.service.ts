...
const createCompanyInDb = async (data: CreateCompanyInput, userId: string) => {
...
// Update company
const updateCompanyInDb = async (id: string, data: UpdateCompanyInput) => {
...
const verifyCompanyInDb = async (id: string, isVerified: boolean) => {
  const company = await prisma.company.findUnique({ where: { id } });
  if (!company) {
    throw new AppError("Company not found", 404);
  }
...
const getUserCompaniesFromDb = async (userId: string) => {
...
const addCompanyMemberInDb = async (
  companyId: string,
  userId: string,
...
const removeCompanyMemberFromDb = async (companyId: string, userId: string) => {
...
// Get company members
const getCompanyMembersFromDb = async (companyId: string) => {
  const company = await prisma.company.findUnique({ where: { id: companyId } });
  if (!company) {
    throw new AppError("Company not found", 404);
  }
