...
const submitApplicationToDb = async (jobId, candidateId, payload) => {
    ...
    if (sanitizedCoverLetter) {
        if (!validateHTMLLength(sanitizedCoverLetter, 5000)) {
            throw new AppError("Cover letter exceeds maximum length of 5000 characters", 400);
        }
    ...
    try {
        ...
        if (recruiter?.email && candidate?.email) {
            await sendApplicationReceivedEmail(recruiter.email, recruiter.name || "Recruiter", {
    ...
    try {
        ...
        await notificationsService.createNotification(updated.candidate.id, "STAGE_UPDATED", `Application updated — ${updated.job.title}`, `Your application for ${updated.job.title} is now ${updated.stage}`, {
            applicationId: updated.id,
            jobId: updated.job.id,
            stage: updated.stage,
        });
        // Recruiter notification (who performed the change already gets socket event)
        await notificationsService.createNotification(recruiterId, "STAGE_UPDATED", `Applicant updated — ${updated.job.title}`, `${updated.candidate.name || "Candidate"} moved to ${updated.stage}`, {
            applicationId: updated.id,
    ...
    try {
        await notificationsService.createNotification(job.postedById, "APPLICATION_RECEIVED", `New application: ${job.title}`, `${application.candidate?.name || "Candidate"} applied for ${job.title}`, {
            jobId: job.id,
            applicationId: application.id,
        });
    }
...
const getMyApplicationsFromDb = async (candidateId, page = 1, limit = 10) => {
...
const getApplicantsForJobFromDb = async (jobId, recruiterId, page = 1, limit = 10) => {
    const job = await prisma.job.findUnique({
        where: { id: jobId },
        select: {
            id: true,
...
const moveApplicantStageInDb = async (applicationId, recruiterId, payload) => {
    const existing = await prisma.application.findUnique({
    ...
    try {
        if (updated.candidate.email) {
            await sendStageChangeEmail(updated.job.company.id, updated.candidate.email, {
...
export const bulkMoveApplicantStagesInDb = async (applicationIds, recruiterId, stage, reason) => {
    const admin = await isUserAdmin(recruiterId);
    const results = [];
...
export const exportApplicantsToCSVFromDb = async (jobId, recruiterId) => {
    const job = await prisma.job.findUnique({
        where: { id: jobId },
    ...
    const admin = await isUserAdmin(recruiterId);
    if (!admin && job.postedById !== recruiterId) {
        throw new AppError("You do not have permission to export applicants for this job", 403);
    }
    ...
    applications.forEach((app) => {
        csvRows.push(`"${app.candidate.name}","${app.candidate.email}","${app.stage}","${app.createdAt.toISOString()}","${app.source}"`);
    });
    return csvRows.join("\\n");
};
export const addApplicationNoteToDb = async (applicationId, authorId, content) => {
    ...
    if (!admin && application.job.postedById !== authorId) {
        throw new AppError("You do not have permission to add notes to this application", 403);
    }
...
export const getApplicationNotesFromDb = async (applicationId, recruiterId) => {
    const application = await prisma.application.findUnique({
        where: { id: applicationId },
        select: {
            job: { select: { postedById: true } },
    ...
    if (!admin && application.job.postedById !== recruiterId) {
        throw new AppError("You do not have permission to view notes for this application", 403);
    }
...
export const getApplicationTimelineFromDb = async (applicationId, userId) => {
    const application = await prisma.application.findUnique({
        where: { id: applicationId },
        select: {
            candidateId: true,
...
export const withdrawApplicationFromDb = async (applicationId, candidateId) => {
    const application = await prisma.application.findUnique({
        where: { id: applicationId },
    });
    if (!application) {
...
export const getKanbanBoardFromDb = async (jobId, recruiterId) => {
    const job = await prisma.job.findUnique({
        where: { id: jobId },
        select: { postedById: true },
    });
...
export const updateApplicationLabelsInDb = async (applicationId, recruiterId, labels) => {
    const application = await prisma.application.findUnique({
        where: { id: applicationId },
...
export const addLabelToApplicationInDb = async (applicationId, label, recruiterId) => {
    const application = await prisma.application.findUnique({
        where: { id: applicationId },
...
export const removeLabelFromApplicationInDb = async (applicationId, label, recruiterId) => {
    const application = await prisma.application.findUnique({
        where: { id: applicationId },
        select: { job: { select: { postedById: true } }, labels: true },
    });
