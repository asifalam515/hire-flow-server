...
import { sendInterviewScheduledEmail, sendOfferExtendedEmail, } from "./notification.service";
/**
 * Schedule an interview for an application
 */
export const scheduleInterview = async (applicationId, recruiterId, data) => {
    ...
    if (!admin && application.job.postedById !== recruiterId) {
        throw new AppError("You do not have permission to schedule interviews for this application", 403);
    }
    // Create interview
    ...
    try {
        if (application.candidate.email) {
            await sendInterviewScheduledEmail(application.candidate.email, application.candidate.name || "Candidate", {
 ...
 */
export const sendOfferEmail = async (applicationId, salaryOffer) => {
    const application = await prisma.application.findUnique({
        where: { id: applicationId },
        include: {
            candidate: {
    ...
    try {
        await sendOfferExtendedEmail(application.candidate.email, application.candidate.name || "Candidate", {
            candidateName: application.candidate.name || "Candidate",
            candidateEmail: application.candidate.email,
            jobTitle: application.job.title,
            companyName: application.job.company.name,
