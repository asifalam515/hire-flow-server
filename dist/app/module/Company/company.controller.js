...
export const createCompany = asyncHandler(async (req, res) => {
    const { name, slug, logoUrl, bannerUrl, website, linkedinUrl, description, industry, size, founded, country, city, } = req.body;
    if (!name || !slug) {
...
export const updateCompany = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { name, logoUrl, bannerUrl, website, linkedinUrl, description, industry, size, founded, country, city, isActive, } = req.body;
    // Check if user is company owner or admin
    const company = await companyService.getCompanyByIdFromDb(id);
    const isOwner = company.members.some((m) => m.userId === req.user?.id && m.isOwner);
...
export const getUserCompanies = asyncHandler(async (req, res) => {
    const companies = await companyService.getUserCompaniesFromDb(req.user?.id);
    res.status(200).json({
        success: true,
        data: companies,
    });
...
export const addCompanyMember = asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    const company = await companyService.getCompanyByIdFromDb(id);
    const isCompanyOwner = company.members.some((m) => m.userId === req.user?.id && m.isOwner);
    if (!isCompanyOwner && req.user?.role !== "ADMIN") {
        throw new AppError("You do not have permission to add members to this company", 403);
    }
    const member = await companyService.addCompanyMemberInDb(id, userId, isOwner || false);
    res.status(201).json({
        success: true,
        data: member,
...
export const removeCompanyMember = asyncHandler(async (req, res) => {
    ...
    const isCompanyOwner = company.members.some((m) => m.userId === req.user?.id && m.isOwner);
    if (!isCompanyOwner && req.user?.role !== "ADMIN") {
        throw new AppError("You do not have permission to remove members from this company", 403);
    }
